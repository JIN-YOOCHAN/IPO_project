def modeling():
    ...
    with open('db_to_modeling.csv', 'w') as f:
    f_csv = csv.writer(f)
    ...
    df = pd.read_csv("./db_to_modeling.csv")
    df
    ...
    dists = {
            ...
            'randomforestregressor__max_depth':range(5,10,1),
            'randomforestregressor__max_features':uniform(0,1),
            'randomforestregressor__min_samples_leaf':range(5,10,1)
            }
    clf = RandomizedSearchCV(pipe, param_distributions=dists, n_iter=3, cv=5,
    ...
    result["Profit"][result["Profit"]==99999999] ="???"
    result.head(10)
    ...
    df_to_db.to_csv("to_postgre.csv")
